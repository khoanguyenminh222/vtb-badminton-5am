 ...
 * @param {Array<{displayName: string}>} members - Danh sách thành viên được chọn
 * @returns {Promise<{hasExistingData: boolean, existingMembers: string[], conflicts: Array<{memberId: string, memberName: string, existingValue: string|number}>}>}
 */
export async function checkExistingPayments(sheetUrl, dateStr, members) {
  ...
  if (dateColIndex === -1) {
    ...
    return { hasExistingData: false, existingMembers: [], conflicts: [] };
  }
